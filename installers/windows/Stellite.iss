; Stellite GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Monero Project
; Copyright (c) 2017-2018, The Stellite Project
; See LICENSE

[Setup]
AppName=Stellite GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.13.0.4
DefaultDirName={pf}\Stellite GUI Wallet
DefaultGroupName=Stellite GUI Wallet
UninstallDisplayIcon={app}\Stellite-wallet-gui.exe
...
LicenseFile=LICENSE
AppPublisher=The Stellite Developer Community
AppPublisherURL=https://www.stellite.cash
...
; Background info:
; This installer indeed asks for admin rights so the Stellite files can be copied to a place where they have at least
...
; As of version 0.13.0.4, the Stellite .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Stellite.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Stellite GUI wallet exe and guide
Source: "bin\Stellite-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-GUI-guide.pdf"; DestDir: "{app}"; Flags: ignoreversion

; Stellite GUI wallet log file
...
Source: "bin\Stellite-gen-trusted-multisig.exe"; DestDir: "{app}"; Flags: ignoreversion

; Stellite wallet RPC interface implementation
Source: "bin\Stellite-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Stellite daemon
Source: "bin\Stellited.exe"; DestDir: "{app}"; Flags: ignoreversion

; Stellite daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "Stellite-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Stellite blockchain utilities
Source: "bin\Stellite-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-mark-spent-outputs.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-usage.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-ancestry.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\Stellite-blockchain-depth.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\Stellite-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\Stellite-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitStellite');
  s := 'The default folder to store the Stellite blockchain is ' + blockChainDefaultDir;
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\Stellite-project\Stellite-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Stellite default location
...
begin
  Result := BlockChainDir('') + '\bitStellite.log';
...
var s: String;
begin
  s := ExpandConstant('{app}\Stellite-wallet-gui.log');
  if Pos(' ', s) > 0 then begin
    // Quotes needed for filename with blanks
    s := '"' + s + '"';
  end;
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "Stellite-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Stellite daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\Stellited.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\Stellite-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Stellite GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\Stellite-wallet-gui.exe"; Parameters: {code:WalletFlags}
Name: "{group}\GUI Wallet Guide"; Filename: "{app}\Stellite-GUI-guide.pdf"; IconFilename: "{app}\Stellite-wallet-gui.exe"
...
Name: "{group}\Utilities\Stellite Daemon"; Filename: "{app}\Stellited.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\Stellite-wallet-cli.exe"; WorkingDir: "{userdocs}\Stellite\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Stellite\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\Stellite-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\Stellite-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM Stellited.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\Stellite-wallet-gui.exe"; Parameters: {code:WalletFlags}; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\Stellite-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\Stellite-project\Stellite-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\Stellite-project\Stellite-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};
